...
class Recipe {
  ...
  public static fromObject(recipeObject: {
    id: number;
    title: string;
    image: string;
    ...
    rating: number | null;
    addedDateTime: Timestamp | null;
  }) {
    return new Recipe(
      recipeObject.id,
      recipeObject.title,
      recipeObject.image,
      recipeObject.isFavorite ?? false,
      recipeObject.isTried ?? false,
      recipeObject.rating ?? -1,
      recipeObject.addedDateTime ?? Timestamp.now()
    );
  }
